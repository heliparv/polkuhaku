...
class Testilabyrintti:
    ...
    Luokan muuttuja 'labyrintti' sisältää ASCII-muotoisen labyrinttigrafiikan'''

    def __init__(self, leveys = 1, korkeus = 1, tiheys = 'harva'):
        '''Käyttää luo_labyrintti funktiota luodakseen labyrintin annettujen leveyden,
        korkeuden ja harva, tiheä vai siksak määrityksen perusteella. Luo harvan
        labyrintin jos käyttäjän syöte ei vastaa mitään labyrinttityyppiä tai syötettä
        ei ole.'''

        self.labyrintti = []
        if leveys > 0 and korkeus > 0:
            if tiheys == 'siksak':
                self.luo_labyrintti(leveys, korkeus, siksak())
            elif tiheys == 'tiheä':
                self.luo_labyrintti(leveys, korkeus, tihea())
            else:
                self.luo_labyrintti(leveys, korkeus, harva())
        else:
            self.luo_labyrintti(1, 1, self.harva())

    def luo_labyrintti(self, leveys: int, korkeus: int, kuvio):
